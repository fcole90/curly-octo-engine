...
# --- Movielens functions ---
def load_ml_movies(ml_movies_file_path=None):
    """
    Loads the ml_movies movies file as a list of dictionaries.
    ...
    with open(ml_ratings_file_path, "r")\
        ...
        for line in ml_ratings_file:
            ...
            if user_id not in ml_ratings.keys():
                ml_ratings[user_id] = list()

            if include_timestamp:
                ml_ratings[user_id].append({movie_id: (rating, timestamp)})
            else:
                ml_ratings[user_id].append({movie_id: rating})
